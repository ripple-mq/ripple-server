...
.PHONY: install
## `install`: Fetch dependencies needed to run `ripple-server`
install:
...
.PHONY: run
## :
## `run`: Run `ripple-server` in production mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `ripple-server`
docker-gen:
